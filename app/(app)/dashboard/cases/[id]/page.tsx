...
export default async function CaseDetailPage({ params }: { params: { id: string } }) {
  ...
  if (caseRow.status === "draft") {
    if (caseRow.intake_version === 2) {
      redirect(`/case/${caseRow.id}/build`);
    }
    ...
    <div>
      <div className="app-page-head">
        <div>
          ...
          <h1>{caseRow.plaintiff_name?.trim() || "You"} vs. {caseRow.defendant_name}</h1>
          ...
          {letter ? (
            <>
              ...
              <div className="app-detail-actions">
                <Link href={`/dashboard/cases/${caseRow.id}/letter`} className="btn btn-dark">
                  Open letter editor
                </Link>
              </div>
            </>
